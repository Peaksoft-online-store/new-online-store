        ...
        return (<div key={id} className='setProduct_main'>
          <div className='setProduct_main2'>
            ...
            <div className='setProduct_second'>
              <h4 >{el.title}</h4>
              <p className='setProduct_price'><b> ${el.price}</b></p>
              <div className='setProduct_category'><i>{el.category}</i></div>
              <p className='setProduct_description'>{el.description}</p>
              {user ?
                <Link to="/basket">
                  <Button variant="contained" onClick={() => dispatch(selectedProducts(el))}
